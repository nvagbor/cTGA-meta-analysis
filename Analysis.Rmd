    ...
    for (ll in time_points) {
      ...
      summary_forest[[endpoints[jj]]][[kk]][[ll]] <- 
        ...
        transmute(
          ...
          surgery=kk, 
          time=case_when(ll=="n_inhospital_30d"~"Inhospital/30 days", 
                         ll=="n_1y"~"1 year", 
                         ll=="n_5y"~"5 years", 
                         TRUE~"10 years"),
          incidence=pred*100, 
...
baseline <- mutate(
    baseline, 
    age_subgroup=case_when(median_age_y<5~"<5", TRUE~"5+"), 
    pm_subgroup=case_when(
      pm<20~"Less than 20", 
      pm>20~"20+", 
      TRUE~NA_character_),  
    moderate_tr_subgroup=case_when(moderate_tr<20~"Less than 20", moderate_tr>20~"20+", TRUE~NA_character_), 
    pab_subgroup=case_when(preoperative_pab<30~"Less than 30", preoperative_pab>30~"30+", TRUE~NA_character_)) %>% 
...
subgroup_var_labs <- list(
  "Anatomic"=c("Median age (years)", "Moderate TR (%)", "Preoperative PAB (%)", "Pacemaker use(%)", "UNSD region"), 
  "Physiologic"=c("Median age (years)", "Moderate TR (%)", "Pacemaker use(%)", "UNSD region") 
)
...
for(jj in seq_along(datasets)){
  for(kk in names(subgroup_var_labs)){
    for(ll in time_points){
      ...
      png(paste0("Subgroup/", Endpoints[jj], "_", kk, "_", ll, ".png"), 
             units     = "in", 
             width     = 9, 
             height    = 5, 
             pointsize = 11,  
